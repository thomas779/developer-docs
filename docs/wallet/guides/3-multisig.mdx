...
description: Learn how to create a multisig wallet and sign with all parties a userOperation
...
- If you need, the full example of this tutorial on [Github](https://github.com/candidelabs/abstractionkit/tree/main/examples/SafeAccountExamples/CreateAccountAndSendTransactionMultiSigners)
...
```bash
mkdir safe-smart-multisig
cd safe-smart-multisig
```
...
You've now sent your first user operation with a multisig wallet! If everything went well, you should see the bundler returning a user operation receipt
