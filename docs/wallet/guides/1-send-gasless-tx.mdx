...
If you are coming from the [previous guide](../getting-started), you can skip directly to [step 5](./#step-5-get-paymaster-data).
...
- An API key to Candide's Paymaster. [Visit Dashboard](https://dashboard.candide.dev)
- Special access to a global policy. [Send us telegram message](https://t.me/heymarcopolo)
- If you need, the full example of this tutorial on [Github](https://github.com/candidelabs/abstractionkit/tree/main/examples/CandideAccountExamples/CreateAccountAndSendGaslessTransaction)

:::info
Gas is currently sponsored by Candide. In the next major release, wallets will get access to policies established by dapps on mainnet. 

Anyone will be able to setup a gas policies for users of wallets with Candide's Paymaster.
:::
...
We will be constructing the callData using [createSendEthCallData](../../abstractionkit/candide-account/#createcalldata), which returns the instructions to send our transaction on-chain. This user operation will make a transfer of 5 wei to a random address
