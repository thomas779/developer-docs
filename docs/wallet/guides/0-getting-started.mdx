...
The [callData](https://eips.ethereum.org/EIPS/eip-4337#specification) contain the instructions to send our transaction on-chain. This user operation will make a transfer of 5 [wei](https://ethereum.org/en/developers/docs/intro-to-ether/#denominations) to a random address
...
### Step 4: Estimate Gas
Estimates the gas required for the user operation using the Bundler instance.
...
user_operation.preVerificationGas = toBeHex(estimation.preVerificationGas);

// multiple by three to avoid out of gas error during validation for verificationGas - 
// note: will be removed once the bundler returns an accurate estimates for verification gas
user_operation.verificationGasLimit = toBeHex(
  Math.round(Number(estimation.verificationGas) * 3)
);
...
if (feeData.maxFeePerGas && feeData.maxPriorityFeePerGas) {
  user_operation.maxFeePerGas = toBeHex(feeData.maxFeePerGas);
  user_operation.maxPriorityFeePerGas = toBeHex(feeData.maxPriorityFeePerGas);
}
```
