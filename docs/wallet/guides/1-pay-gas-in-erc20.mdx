...
2. Use the `getPaymasterCallDataForPayingGasWithErc20()` function to get the paymaster data to pay gas in erc-20s using. Don't forget to add your paymaster url in your `.env` file.
...
const paymasterAndDataResult = await paymaster.getPaymasterCallDataForPayingGasWithErc20(
  user_operation,
  CTTAddress // ERC-20 token to pay gas with
);
