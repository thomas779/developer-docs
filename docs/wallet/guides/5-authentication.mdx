...
const safeUserOperation = SafeAccount.getUserOperationEip712Hash(userOperation, chainId);

const signature = signer.signingKey.sign(safeUserOperation).serialized;
...
const safeUserOperation = SafeAccount.getUserOperationEip712Hash(
  userOperation,
  chainId
) as `0x${string}`;

const signature = await walletClient.sign({ hash: safeUserOperation });
