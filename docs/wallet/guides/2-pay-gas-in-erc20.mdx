...
If you are coming from the [previous guide](../send-gasless-tx), you only need adjust the paymaster step. Skip directly to [step 5](./#step-5-get-paymaster-data).
...
- An API key to access Candide's Paymaster. [Visit dashboard](https://dashboard.candide.dev)
- Some Candide Test Tokens (CTT). Request CTT on [discord](https://discord.gg/22Xps7M5CJ) or [telegram](https://t.me/heymarcopolo)

:::info
You can find the full example of this tutorial on [Github](https://github.com/candidelabs/abstractionkit/tree/main/examples/SafeAccountExamples/CreateAccountAndSendTransactionUsingTokenPaymaster)
:::
...
```
:::info
We host free [bundlers](../../bundler/rpc-endpoints) endpoints, you do not need to create an account.
:::
...
:::info
You can override any of the returned values using the overrides parameter.
:::
...
const userOperation = await paymaster.createTokenPaymasterUserOperation(
    smartAccount,
    userOperation,
    tokenAddress,
    bundlerUrl,
...
:::info
Don't forget to fund the account with ERC-20s to pay for gas. We are using Candide Test Tokens (CTT). Request some on [telegram](https://t.me/heymarcopolo) or [discord](https://discord.gg/22Xps7M5CJ).
:::
...
3. Track the userOperation and wait its inclusion onchain 
```ts
// highlight-start
...
You've now sent your first user operation with a token paymaster! If everything went well, you should see the bundler returning a user operation receipt
