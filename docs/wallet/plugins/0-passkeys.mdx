...
This step follows the same flow as the normal Safe flow with  [createUserOperation](../../abstractionkit/safe-account/#createuseroperation), with the addition of the `expectedSigners` overrides 
...
- Add a Passkeys owner to an existing account using [`addOwnerWithThreshold`](/wallet/abstractionkit/safe-account-v3/#createaddownerwiththresholdmetatransactions)
...
let smartAccount = SafeAccount.initializeNewAccount(
    [webauthPublicKey],
    // highlight-start
    { eip7212WebAuthnPrecompileVerifierForSharedSigner: SafeAccount.DEFAULT_SECP256R1_PRECOMPILE_ADDRESS } 
    // highlight-end
)
```
...
let userOperation = await smartAccount.createUserOperation(
    ...
    bundlerURL,
    {
        expectedSigners:[webauthPublicKey],
        // highlight-start
        eip7212WebAuthnPrecompileVerifier: SafeAccount.DEFAULT_SECP256R1_PRECOMPILE_ADDRESS
        // highlight-end
    }
);
```
