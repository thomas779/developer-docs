...
import { userOperationReturnParamsV06, userOperationParamsV07 } from "/src/data/userOperation"
import { 
...
JSON-RPC API reference for Candide's Verifying Paymaster V3 service

:::info
V3 is not available to the public yet.
:::
...
Sends a UserOperation to a paymaster for off-chain verification. If approved, it will return the `paymasterData`. It will also return gas overrides when the mode for gas sponsoriship uses a Gas Policy.

- **Pay Gas in ERC-20s**: No gas overrides are returned
- **Gas Policy**: Mandatory `callGasLimit`, `verificationGasLimit`, `preVerificationGas`, `maxFeePerGas`, and `maxPriorityFeePerGas` fields are returned.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td align="left">
        <p>Object</p>
        <DataTable items={userOperationParamsV07} />
      </td>
    </tr>
    <tr>
      <td align="left">entrypoint</td>
